        ...
        public ImportSummaryDto<SubCategoryMasterRowDto> ValidateExcel(HttpPostedFileBase file)
        {
            ...
            using (var stream = new MemoryStream())
            {
                ...
                using (var workbook = new XLWorkbook(stream))
                {
                    ...
                    // =============================================
                    var expectedHeaders = new List<string> { "Code", "SubCategoryName" };
            ...
            foreach (var row in rows)
            {
                ...
                {
                    row.IsValid = false;
                    row.ErrorMessage1 = "Sub Category Code cannot exceed 10 characters.";
                }
                else if (row.SubCategoryName.Length > 50)
                {
                    row.IsValid = false;
                    row.ErrorMessage1 = "Sub Category Name cannot exceed 50 characters.";
                }
